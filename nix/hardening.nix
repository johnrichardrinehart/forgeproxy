...
in
{
  ...
  options.services.forgecache-hardening = {
    allowSMT = lib.mkOption {
      ...
      description = ''
        ...
        Enabling SMT doubles the visible vCPU count on EC2 but exposes the
        system to side-channel attacks (e.g. MDS, L1TF). Only enable this
        when the performance gain outweighs the security risk.
      '';
    };
  };
  ...
  config = lib.mkMerge [
    ...
    # ══════════════════════════════════════════════════════════════════════
    # SMT toggle -- profiles/hardened.nix disables SMT by default, halving
    # the visible vCPU count. Allow overriding for performance-sensitive
    # workloads that accept the side-channel risk.
    # ══════════════════════════════════════════════════════════════════════
    (lib.mkIf cfg.allowSMT {
