...
let
  cfg = config.services.forgecache.compliance.regulated;
  forgecacheModuleLoaded = options.services ? forgecache && options.services.forgecache ? package;
in
{
  options.services.forgecache.compliance.regulated = {
    enable = lib.mkEnableOption "Regulated compliance controls (FIPS SSH, AU-2/AU-12 audit rules, FIPS build)";
  };
  ...
  config = lib.mkMerge (
    [
      (lib.mkIf cfg.enable {
        ...
        # ══════════════════════════════════════════════════════════════════════
        # Regulated AU-2 / AU-12 audit rules
        # ══════════════════════════════════════════════════════════════════════
