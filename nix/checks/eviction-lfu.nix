...
let
  ...
  # ---------------------------------------------------------------------------
  # forgeproxy configuration YAML — LFU eviction policy
  # ---------------------------------------------------------------------------
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    with subtest("Seed Valkey with clone_count and bundle_list_key"):
        # Wait for each repo to be durably published locally, then stop
        # forgeproxy before seeding test metadata so post-clone RepoInfo
    ...
    # ── Verify the forgeproxy service is running with LFU config ──────────
    with subtest("forgeproxy service is active with LFU eviction policy"):
